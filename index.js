const MyForm = (function(formId) {
  ...
  const setClassNameForMyFormFields = function(fields, className) {
    for (let field of fields) {
      document.querySelector('form#' + formId + ' > input[name="' + field + '"]').className = className;
    }
  }
  ...
  const sendRequest = function() {
    ...
    request.open(ajaxRequestMethod, myForm.action, true);
    ...
    request.send(ajaxRequestMethod == "POST" ? new FormData(myForm) : null);
  }

  const validateFio = function(fio) {
    // Проверка поля ФИО
    var words = fio.split(/\s+/),
  ...
  const validateEmail = function(email) {
    // Проверка поля Email
    var template_is_matched = email.match(/^.+@[^@\s\.]+\.[^@\s\.]+$/),
      domain_is_valid = true,
      local_is_valid = true;
    if (template_is_matched) {
      var email_splited = email.split(/@/),
        local = email_splited[0],
        domain = email_splited[1];
      const regex1 = /^(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")$/;
      local_is_valid = local.match(regex1);
